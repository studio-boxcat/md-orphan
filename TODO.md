...
`walk_cache.rs` compares `cache.canonical_root` against `canonical_root.to_string_lossy()` (line 87) and hashes via the lossy form for the cache filename. Two distinct paths with non-UTF-8 bytes could lossy-collide, returning a wrong-repo cache. Not a hazard for current users (macOS APFS is UTF-8; Linux dev paths typically are too). Fix would be to hash raw `OsStr` bytes via `as_encoded_bytes()` and reject non-UTF-8 paths from the equality check, or store both. Defer.
